...
def lift_lockdown_paper():
    ...
    pop_scale = 1000
    test_kwargs = {'sympt_test': 0.0, 'quar_test': 10.0, 'sensitivity': 1.0, 'test_delay': 0, 'loss_prob': 0}
        ...
        f'{tn}_ineff': {
            ...
            'pars': {
                'interventions': pre_lockdown_interventions + [
                    ...
                    screen(daily_screens=np.array([20e6]*180) / pop_scale,
                ...
                'interventions': pre_lockdown_interventions + [
                   # new post-lockdown interventions
                    ...
                    cv.contact_tracing(start_day='2020-05-04', end_day='2020-09-30',
                                       trace_probs={'h': 1, 's': 0, 'w': 0.8, 'c': 0.1},
                                       trace_time={'h': 1, 's': 7, 'w': 7, 'c': 7}),
                    cv.change_beta([35, 182], [0.9, 1], layers='w'),  # Reduce beta by by 10% for face masks
                    cv.change_beta([35, 182], [0.9, 1], layers='c')]  # Reduce beta by by 10% for face masks
    }
        } for tn in testnos
    }
...
def run_sim():
    ...
    interventions = [cv.clip_edges(start_day=0, end_day=35, change={'w': 0.3}), # Reduce work by 70% til May 4
                                  cv.test_num(daily_tests=np.array([200]*180) / pop_scale, start_day=0, end_day=35, **test_kwargs),
                                  cv.contact_tracing(start_day=0, end_day=35,
                                                     trace_probs={'h': 1, 's': 0, 'w': 0.8, 'c': 0.1},
                                                     trace_time={'h': 1, 's': 7, 'w': 7, 'c': 7}),
                                # new post-lockdown interventions
                                screen(daily_screens=np.array([200e3]*180) / pop_scale,
                                       start_day=35),
                                quarantine_severe(start_day=35),
                                cv.test_num(daily_tests=np.array([200] * 180) / pop_scale, start_day=35,
                                            end_day=180, **test_kwargs),
                                cv.contact_tracing(start_day=35, end_day=180,
                                                   trace_probs={'h': 1, 's': 0, 'w': 0.8, 'c': 0.1},
                                                   trace_time={'h': 1, 's': 7, 'w': 7, 'c': 7})]
    ...
    sim.run()
#    sim.plot(do_save=1, do_show=0, to_plot=to_plot, fig_path=f'results/nigeria_scenarios_paper1', n_cols=2, fig_args=fig_args)
